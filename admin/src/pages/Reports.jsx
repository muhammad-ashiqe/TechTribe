...
const Reports = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const endpoint = filterType === 'post' 
  ...
  const handleStatusUpdate = async (reportId, newStatus) => {
    try {
      const res = await fetch(`http://localhost:7000/api/admin/reports/${reportId}/status`, {
        method: 'PATCH',
        headers: {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs flex items-center gap-1 ${statusStyles[status]}`}>
  ...
  const filteredReports = reports.filter(report => {
    ...
    return (
      ...
      (filterType === 'post' 
        ? report.reportedPost?.description?.toLowerCase().includes(searchLower) ||
          report.reportedPost?.user?.firstName?.toLowerCase().includes(searchLower) ||
          report.reportedPost?.user?.lastName?.toLowerCase().includes(searchLower)
    ...
    return (
      <div className="p-8 bg-gray-900 min-h-screen flex items-center justify-center">
        <div className="max-w-md bg-gray-800 p-6 rounded-2xl text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-100 mb-2">Error Loading Reports</h2>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                Post Reports ({filteredReports.filter(r => r.type === 'post').length})
              </button>
              <button
              ...
              >
                ...
                User Reports ({filteredReports.filter(r => r.type === 'user').length})
              </button>
            </div>
          </div>
          ...
          {filteredReports.map(report => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="relative">
                  <div className="flex items-center gap-2">
                    ...
                    <button 
                      onClick={() => setOpenDropdownId(openDropdownId === report._id ? null : report._id)}
              ...
              <div className="mb-4 bg-gray-800/50 p-4 rounded-xl border border-gray-700">
                <p className="text-gray-300 text-sm italic">"{report.reason}"</p>
              </div>
                ...
                <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-700">
                  ...
                  <div className="mt-3 text-sm space-y-1">
                    {report.reportedUser?.headline && (
                      <p className="text-gray-400">{report.reportedUser?.headline}</p>
                    )}
